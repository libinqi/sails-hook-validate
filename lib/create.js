...
module.exports = function (model, validateCustom) {
    ...
    function create(values, callback) {
        ...
        if (model.definition.isDelete) {
            ...
            for (var attr in model.definition) {
                if (model.definition[attr].unique != undefined && model.definition[attr].unique == false) {
            ...
            if (hasProp) {
                attrs['isDelete'] = false;

                model.find.call(model, attrs, {}, function (error, result) {
                    if (error) {
                        //process sails invalidAttributes and
                        //attach Errors key in error object
                        //as a place to lookup for our
                        ...
                        callback(WLValidationError.patch('column unique'));
                    }
                    else {
						createFn();
                    }
